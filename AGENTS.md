...
Runs automatically when Codex finishes a task. Scans all project files.
...
Global notification hook (lives at `~/.Codex/hooks/notify.sh`). Sends a system notification when:
- Codex finishes a task (Stop)
- Codex sends a notification (Notification)
- Codex is about to ask the user a question (AskUserQuestion) — sends an idle prompt alert
...
**Plugins:** Warp terminal integration (`warp@Codex-warp`)

**Status line:** Custom status line via `~/.Codex/statusline-command.sh`
